...
type ReceiveFromStep = Extract<QuestStep, { type: 'receive_from' }>;

export interface ReceiveFromStepEditorHandle {
  getStep: () => ReceiveFromStep;
}

interface Props {
  initialStep?: ReceiveFromStep;
}
...
export const ReceiveFromStepEditor = forwardRef<ReceiveFromStepEditorHandle, Props>(
  function ReceiveFromStepEditor({ initialStep }, ref) {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="w-32">
          ...
          <input type="number" min={1} value={count} onChange={(e) => setCount(Number(e.target.value))} className={INPUT} />
        ...
        <div>
          ...
          <input type="text" value={message} onChange={(e) => setMessage(e.target.value)} placeholder="例：請跟園丁拿藥草。" className={INPUT} />
